...
with right:
    ...
    if st.button("Generate Review Comment"):
        with st.spinner("Azure OpenAIで審査コメントを生成中..."):
            try:
                ai_comment = generate_azure_review_comment(row, top_shap_text)
                st.success("Generated by Azure OpenAI")
                st.write(ai_comment)
            except Exception as e:
                st.error("Azure OpenAI call failed")
                st.exception(e)
